...
{
    public class TypeReceiptController : ApiController
    {
        public IHttpActionResult Get(int ID)
        {
            int iID = int.Parse(ID.ToString());
            DataTable dt = SqlHelper.ExecuteDataset("TypeReceiptGetByID", iID).Tables[0];
        ...
        {
            DataTable dt = SqlHelper.ExecuteDataset("TypeReceiptGetAll").Tables[0];
        ...
        public HttpResponseMessage Add([FromBody]JObject jfrom)
        {
            ...
            TypeReceiptModel tp = new TypeReceiptModel();
            tp.Name = sName;
            tp.Desc = sDesc;

            try
            {
                SqlHelper.ExecuteNonQuery("TypeReceiptAdd", tp.Name, tp.Desc);
            }
            catch
            {
                return Request.CreateResponse(HttpStatusCode.OK, 0);
        ...
        public HttpResponseMessage Edit([FromBody]JObject jfrom)
        {
            ...
            try
            {
                SqlHelper.ExecuteNonQuery("TypeReceiptUpdate", tp.ID, tp.Name, tp.Desc);
            }
            catch
            {
                return Request.CreateResponse(HttpStatusCode.OK, 0);
